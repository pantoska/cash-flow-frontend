<template>
  <div class="entry-list-container">
    <ul>
      <li v-for="(item, index) in entryDataList" :key="item.id">
        <EntryDateLimiter
          :text="getActualDate(item.date)"
          v-if="printDate(index)"
        />
        <EntryLimiter v-else />
  ...
  methods: {
    ...
    printDate: function (index) {
      if (index === 0) {
        ...
        return (
          new Date(this.entryDataList[index].date) <
          new Date(this.entryDataList[index - 1].date)
        );
      }
    },
    getActualDate(date) {
