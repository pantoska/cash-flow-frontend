<template>
  <div class="entry-box d-flex">
    ...
    <div class="entry-data-box flex-grow-1" :class="getMoneyColor" v-if="!openedEntryView">
      <EntrySmall v-if="entrySize === 'small'" :entry-data="entryData" :amount="getAmount"/>
      <EntryNormal v-else :entry-data="entryData" :amount="getAmount"/>
    </div>
    <div class="entry-data-box flex-grow-1" :class="getMoneyColor" v-else>
      <EntryOpened :entry-data="entryData" :amount="getAmount"/>
    </div>
    <div class="entry-menu-button">
      <div>
        <ChevronDown :size="25" v-if="!openedEntryView" @click="onClickExpandEntryView"/>
...
<script>
  ...
  export default {
    ...
    computed: {
      getCategoryIcon () {
        return this.entryData.categoryName[0]
      },
      getCategoryColor () {
        return 'background-color:' + this.entryData.color
      },
      getAmount () {
        ...
        // }
        return MoneyController.amountToString(this.entryData.amount) + 'zł'
      },
      getMoneyColor () {
